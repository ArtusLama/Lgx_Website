...
- `pnpm dlx nuxt-seo-utils icons --source favicon.svg` - Generates favicons and app icons from a source SVG file. The generated icons will be placed in the `public` directory. Everything else is handled automatically by the `@nuxtjs/seo` module, so no further configuration is needed.
